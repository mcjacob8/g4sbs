...
void MakeGEP_optics_tree( const char *inputfilename, const char *outputfilename, int pflag=0 ){
  ...
  for( int ifile=0; ifile<list_temp->GetEntries(); ifile++ ){
    
    TString fname_temp = ( (TChainElement*) (*list_temp)[ifile] )->GetTitle();
